...
canPass(R1, R2, M):- not (R1 = net, R2 = net), pathClear(R1, R2), M >= 1.
canPass(R1, R2, M):- not (R1 = net, R2 = net), pathClear(R2, R1), M >= 1.
canPass(R1, R2, M):- not (R1 = net, R2 = net), pathClear(R1, R3), canPass(R3, R2, M-1).
canPass(R1, R2, M):- not (R1 = net, R2 = net), pathClear(R3, R1), canPass(R3, R2, M-1).

canScore(R, M):- hasBall(R), pathClear(R, net), M >= 1.
canScore(R, M):- hasBell(R2), canPass(R2, R, M-1), pathClear(R, net).
